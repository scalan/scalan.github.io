...
tool parses this virtualized snippet and produces all the necessary boilerplate. With some limitations, type
virtualization works for a rather rich subset of Scala including trait hierarchies, generic (polymorphic) types and
high-kind type parameters.
...
<a name="Idiom8"></a> 
### Idiom 8: Staged Method Invocation
...
```scala
case class MethodCall(receiver: Exp[_], method: Method, args: List[AnyRef], neverInvoke: Boolean)
                     (val selfType: Elem[Any]) extends Def[Any] 
```
